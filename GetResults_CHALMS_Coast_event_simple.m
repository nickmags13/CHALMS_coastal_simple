...
LANDVALmap=cell(TMAX,length(hind));
CONidmap=cell(TMAX,length(hind));
SUBJRISKmap=cell(TMAX,length(hind));
...
farmrecord=cell(1,length(hind));

DDATA=cell(1,length(hind));
moveouts=cell(length(hind),TMAX);
...
leave_conid=cell(length(hind),TMAX);
leave_lotind=cell(length(hind),TMAX);
leave_lotid=cell(length(hind),TMAX);
leave_coninc=cell(length(hind),TMAX);

reloc_lots=cell(length(hind),TMAX);
reloc_subjrisk=cell(length(hind),TMAX);
reloc_inc=cell(length(hind),TMAX);
reloc_conid=cell(length(hind),TMAX);
reloc_start=cell(length(hind),TMAX);
reloc_end=cell(length(hind),TMAX);
reloc_udiff=cell(length(hind),TMAX);
reloc_mvcost=cell(length(hind),TMAX);
...
for mr=1:length(hind)   % MRUNS*EXPTRUNS
    h=strcmp(sprintf('coast_event_simple_%d_%d.mat',...
    ...
    AVGRENTmap(:,mr)=mat2cell(AVGRENT,NLENGTH*NWIDTH,ones(1,TMAX));
    LANDVALmap(:,mr)=mat2cell(LANDVALUE,NLENGTH*NWIDTH,ones(1,TMAX));
    CONidmap(:,mr)=mat2cell(conidmap,NLENGTH*NWIDTH,ones(1,TMAX));
    SUBJRISKmap(:,mr)=mat2cell(subjriskmap,NLENGTH*NWIDTH,ones(1,TMAX));
    devarea=(cat(2,LTmap{:,mr})~=0);
    pctdev(mr,:)=sum(devarea,1)/(NLENGTH*NWIDTH);
    hmc(mr,:)=consumerstats(4,:);
    Lotrent{mr}=LOTRENT;
    Lotcon{mr}=LOTCON;
    Lotinc{mr}=LOTINC;
    Lotins{mr}=LOTINS;
    Lotlocate(mr)=mat2cell(lotlocate,length(lotlocate),2);
    Lotincome(mr)=mat2cell(cat(1,CONINFO{:,1}),length(CONINFO),1);
    DDATA{mr}=decisiondata;
    moveouts(mr,:)=regmoveouts;
    ...
    %exiting consumer stats
    if isempty(find(cat(1,leave_stats{1,:}),1))==0
        leave_conid(mr,:)=leave_stats(1,:);
        leave_lotind(mr,:)=leave_stats(2,:);
        leave_lotid(mr,:)=leave_stats(3,:);
        leave_coninc(mr,:)=leave_stats(4,:);
    end
    
    % relocation stats
    if isempty(find(relocated,1))==0
%         reloc_lots(mr)=mat2cell(relocated,length(relocated),1);
        reloc_lots(mr,:)=reloc_stats(3,:);
        reloc_subjrisk(mr,:)=reloc_stats(7,:);
        reloc_inc(mr,:)=reloc_stats(4,:);
        reloc_conid(mr,:)=reloc_stats(1,:);
        reloc_start(mr,:)=reloc_stats(2,:);
        reloc_end(mr,:)=reloc_stats(3,:);
        reloc_udiff(mr,:)=reloc_stats(5,:);
        reloc_mvcost(mr,:)=reloc_stats(6,:);
    end
    ...
    clear('consumerstats','vacstats','BUILDTIME','VACLAND','LANDVALUE','AVGRENT','RETURN',...
...
mapdata_store=struct('lot_types',{LTmap},'income_map',{incomemap},...
    ...
    {landsaletime},'average_maps',{AVGMAP},'devprob_map',{VARLAYER},...
    'conidmap',{CONidmap},'subjriskmap',{SUBJRISKmap},'avgrentmap',...
    {AVGRENTmap},'landvaluemap',{LANDVALmap});

relocdata_store=struct('reloc_conid',{reloc_conid},'reloc_start',{reloc_start},...
    'reloc_end',{reloc_end},'reloc_inc',{reloc_inc},'reloc_lots',{reloc_lots},...
    'reloc_mvcost',{reloc_mvcost},'reloc_udiff',{reloc_udiff},'reloc_subjrisk',...
    {reloc_subjrisk},'lv_conid',{leave_conid},'lv_lotind',{leave_lotind},...
    'lv_lotid',{leave_lotid},'lv_coninc',{leave_coninc});
lotdata_store=struct('lotrent',{Lotrent},'lotcon',{Lotcon},'Lotlocate',...
    {Lotlocate},'Lotincome',{Lotincome},'lotinc',{Lotinc},'lotins',{Lotins});
ddata_store=struct('decisiondata',{DDATA},'regmoveouts',{moveouts});
...
landproj_run=cell(MRUNS,EXPTRUNS);
...
land_sales=struct('landsales_time',{landsalerlts_time},'landsales_coast',...
    {landsalerlts_coast},'landsales_cbd',{landsalerlts_cbd},...
    'landsales_all',{landsale_run},'landproj',{landproj_run});
save results_event_ilandscape_030617_rnd_struct mapdata_store htdata_store aggdata_store ...
    run_indices land_sales relocdata_store lotdata_store ddata_store
...
% %%
% % runnamelabel={'AmSlope_06','AmSlope_07','AmSlope_08','AmSlope_09','AmSlope_10'};
...
% runnamelabel={'AmSlope_06','AmSlope_07','AmSlope_08','AmSlope_09','AmSlope_10'};
% runnamelabel={'AmSlope_06','AmSlope_08','AmSlope_10'};
...
% cd C:\Users\nmagliocca\Documents\Matlab_code\CHALMS_coast\figs\event_strmfreq_base
...
%     set(gca,'Visible','off')
    ...
    set(get(gca,'Title'),'Visible','on')
    subplot(2,2,4);
    imagesc(AVGMAP(:,:,TMAX,j));
    set(gca,'xticklabel',[],'yticklabel',[])
%     set(gca,'Visible','off')
    title(sprintf('t=%d',TMAX));
    set(get(gca,'Title'),'Visible','on')
    saveas(h1,sprintf('avg_housetypes_%s',runnamelabel{j}),'png')
    
    
    % plot consumer incomes
    subdevmap=AVGMAP(:,:,TMAX,j);
...
%     set(gca,'Visible','off')
    ...
    set(get(gca,'Title'),'Visible','on')
    saveas(h2,sprintf('final_consumer_income_%s',runnamelabel{j}),'png')
...
%         title(sprintf('Income Distribution %s',strmclim{j}))
%     end
%     saveas(h2_1,'consumer_income_distributions','png')
...
%     set(gca,'Visible','off')
    ...
    set(get(gca,'Title'),'Visible','on')
    saveas(h3,sprintf('build_time_%s',runnamelabel{j}),'png')
    
%     % plot entropy of housing type distribution
%     h4=figure;
%     set(h4, 'Color','white','Position',[1,1,400,400],'Visible','off');
%     plot(10:TMAX,mean(htentropy(batchruns{j},10:TMAX),1),'--k','LineWidth',3)
%     hold on
%     plot(10:TMAX,htentropy(batchruns{j},10:TMAX),'-')
%     legend('Mean Entropy','Individual Model Runs')
%     xlabel('Time Step')
%     ylabel('Entropy of Housing Type Distribution')
%     saveas(h4,sprintf('htentropy_%s',runnamelabel{j}),'png')
...
%     set(gca,'Visible','off')
    ...
    set(get(gca,'Title'),'Visible','on')
    saveas(h5,sprintf('landsales_%s',runnamelabel{j}),'png')
...
%     set(gca,'Visible','off')
    ...
    set(get(gca,'Title'),'Visible','on')
    saveas(h6,sprintf('amprefmap_%s',runnamelabel{j}),'png')
    ...
    plot(10:TMAX,mdnrents(:,10:TMAX),'-');
    title('Median Housing Rents');
    xlabel('Time Step');
    ylabel('Rent');
    legend('HT1','HT2','HT3','HT4','HT5','HT6','HT7','HT8','Location','NorthWest');
    saveas(h7,sprintf('avgrents_%s',runnamelabel{j}),'png')
    
    % development probability map
    h8=figure;
    set(h8, 'Color','white','Position',[700,300,400,400],'Visible','off');
    imagesc(reshape(VARLAYER(:,j)./TMAX,80,80))
    title('Development Probability');
    colorbar
    saveas(h8,sprintf('devprobmap_%s',runnamelabel{j}),'png')
...
%     end
% end
% saveas(h_bt,'summary_build_time','png')
% 
% h_ent=figure;
...
%     end 
% end
% saveas(h_ent,'summary_entropy','png')
% 
% % Average Map
% h_avm=figure;
...
%     end
% end
% saveas(h_avm,'summary_avgmap','png')
% 
% % avg rents
...
%     end 
% end
% saveas(h_inc,'summary_incdist','png')
% 
% h_ls=figure;
...
%     end 
% end
% saveas(h_ls,'summary_landsales','png')
