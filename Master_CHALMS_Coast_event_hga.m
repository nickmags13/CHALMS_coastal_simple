...
% load parameters file from R function that initializes parameter settings
% for each individual i in generation g
load params.mat

MRUNS=30;   %iterations per individual in population

rng default
% poolobj=parpool(10);
% addAttachedFiles(poolobj,{'load_params_hga.m','loadempdata.m',...
%     'parsave_hga.m','distmat.m','load_farmmap.m','load_DIST2CBD_east.m',...
%     'load_distmat.m','calc_prisk.m','calc_eu.m','calc_react.m','params.mat'});
...
%     disp([erun mrun])
    ...
    [am0,am_slope,ampref_max,ampref_min,maxPflood,highrisk,stormfreq,maxdam,...
        ...
        eumodel,lclcoeff,altamen]=load_params_hga(filename);
    %<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
...
%     testrents=(10000/2)*[2.0947    2.1348    2.1311    2.1762    2.2347];
    ...
    coastprox=num2cell(am0*1./exp(am_slope*coastdist));
    alt_coastprox=altamen*am0*1./exp(am_slope*max(coastdist));
...
%     stormdraw=[15 20 25];
%     stormoccur=zeros(1,TMAX);
%     stormoccur(stormdraw(stormfreq,:))=1;
    
    betarisk=alpharisk/sum(Psevere)-alpharisk;
    
    % Percent damage with storm categories 1-5
    housedam=maxdam*10.23749-0.23462*(coastdist*cell2ft/1000)+...
...
%     LOTS(ilotfill)=Nlots(1)+(1:length(ilotfill));
    ...
    %     CONINFO(:,6)=num2cell((ampref_min+(ampref_max-ampref_min)*...
    ...
    CONINFO(:,12)=num2cell(timewght*ones(length(CONINFO(:,1)),1));
...
%         IPrate(cat(1,lotchoice{:,1}),TSTART)=num2cell(insurecost*...
%             housedam(cat(1,lotchoice{:,2})));
%         IPrate(cat(1,lotchoice{:,1}),TSTART)=num2cell(insurecost*...
%                 cat(1,Cdam{cat(1,lotchoice{:,1}),TSTART}));
        IPrate(cat(1,lotchoice{:,1}),TSTART)=num2cell(insurecost-...
                insurecost.*(1-housedam(cat(1,lotchoice{:,2}))./max(housedam)));
                ...
                for ius=1:length(ilotmatch)
                    ...
                    elseif euchoice(conset(cs),ilotmatch(ius)) == 2
                        ...
                        uset(ius)=CONINFO{conset(cs),13}.*...
                            ...
                            IPrate{ilotmatch(ius),TSTART}-insddct+min(insurecov,...
                                subPhousebid(conset(cs),ilotmatch(ius))'),0).^CONINFO{conset(cs),3}).*...
                            (Lottype{ilotmatch(ius),4}.^CONINFO{conset(cs),4}).*...
                            (Lottype{ilotmatch(ius),3}.^CONINFO{conset(cs),5}).*...
                            (Lottype{ilotmatch(ius),7}.^CONINFO{conset(cs),6}))+...
...
%     end
    ...
    for nf=1:Nfarmers
        ...
        farmret=(1-discount)*max(mean(estrent_grad(farmacres).*AVGFARMRETURN-...
            ccost(imax_estrent(farmacres))./z(imax_estrent(farmacres),1)),AVGFARMRETURN);
...
%         IPrate(cat(1,lotchoice{:,1}),t)=num2cell(insurecost*...
%             housedam(cat(1,lotchoice{:,2})));
%         IPrate(cat(1,lotchoice{:,1}),t)=num2cell(insurecost*...
%                 cat(1,Cdam{cat(1,lotchoice{:,1}),t}));
        ...
        % Initial storm risk perception is set to cumulative objective
        % storm probability (if storm severity is not considered)
        CONINFO(inewpop,12)=num2cell(timewght*ones(newpop,1));
        ...
        for c=1:length(iocc)
            ...
            if utilstress_chk(iocc(c)) == 1
                ...
                % %                         (cat(1,Lottype{ilot,7}).^CONINFO{iocc(c),6}));
                % %
                iresist_chk=(eu_base-CONINFO{iocc(c),9} > discount*movethresh*CONINFO{iocc(c),9});
                if iresist_chk == 0
                    dmgstress_chk(iocc(c))=0;
...
%             IPrate(cat(1,lotchoice{inewlots,1}),t)=num2cell(insurecost*...
%                 housedam(cat(1,lotchoice{inewlots,2})));
%             IPrate(cat(1,lotchoice{inewlots,1}),t)=num2cell(insurecost*...
%                     cat(1,Cdam{cat(1,lotchoice{inewlots,1}),t}));
            IPrate(cat(1,lotchoice{inewlots,1}),t)=num2cell(insurecost-...
                insurecost.*(1-housedam(cat(1,lotchoice{inewlots,2}))./max(housedam)));
        end
                ...
                for ius=1:length(ilotmatch)
                    ...
                    elseif euchoice(conset(cs),inewlots(ilotmatch(ius))) == 2
                        ...
                        uset(ius)=CONINFO{conset(cs),13}.*...
                            ...
                            min(insurecov,subPhousebid(conset(cs),ilotmatch(ius))'),0).^CONINFO{conset(cs),3}).*...
                            (Lottype{inewlots(ilotmatch(ius)),4}.^CONINFO{conset(cs),4}).*...
                            (Lottype{inewlots(ilotmatch(ius)),3}.^CONINFO{conset(cs),5}).*...
                            (Lottype{inewlots(ilotmatch(ius)),7}.^CONINFO{conset(cs),6}))+...
                            (1-CONINFO{conset(cs),13}).*...
...
%     end
    ...
    savefname=sprintf('coast_event_simple_%d_%d_%d.mat',g_id,ind_id,mrun);
    parsave_hga(savefname,...
...
[score,p_rents,p_inscov] = calc_fitness(MRUNS,g_id,ind_id,TSTART,TMAX,CDIST);