...
%         rndstr.Substream=mrun;
        ...
        [am0,am_slope,ampref_max,ampref_min,maxPflood,highrisk,stormfreq,maxdam,...
            ...
            eumodel,lclcoeff,altamen,propertytax,taxflag,deltadmg]=...
            load_expmntlparms_event_simple(EXPTRUNS);
        %<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
...
%                 cat(1,Cdam{cat(1,lotchoice{:,1}),TSTART}));
            ...
            for c=1:length(CONINFO(:,1))
                ...
                lccf=lclcoeff(erun);
                if taxflag == 0
                    tax=sum(((1/(1+discount)).^(1:30)).*paskh,2).*basetax';
                elseif taxflag == 1
...
%                     lotchoice{lotid,6}=TMAX+1;
            ...
            TAXREV(TSTART)=sum(sum(cat(1,lotchoice{ifilled,7})*...
                ((1/(1+discount)).^(1:30)),2).*basetax);
...
%                 wtpland(iNfarmers(rf),t)=sum(subWTPMAP(ifarmcalc))/length(ifarmcalc);
            ...
            elseif taxflag == 0 && stormoccur(t) == 1
                Cdam(1:Nlots(t),t)=num2cell((1+deltadmg(erun)*t)*cat(1,lotchoice{:,7}).*...
                    (0.01*housedam(cat(1,lotchoice{:,2}))));
            elseif taxflag == 0 && stormoccur(t) == 0
                ilaststorm=find(stormoccur(t-1:-1:1) == 1,1,'first');
                if isempty(find(ilaststorm,1)) == 1
                    Cdam(1:Nlots(t),t)=num2cell((1+deltadmg(erun)*t)*...
                        cat(1,lotchoice{:,7}).*(0.01*housedam(cat(1,lotchoice{:,2}))));
                else
                    Cdam(1:Nlots(t),t)=num2cell((1+deltadmg(erun)*(t-ilaststorm))*...
                        cat(1,lotchoice{:,7}).*(0.01*housedam(cat(1,lotchoice{:,2}))));
                end
            end
...
%                 paskh=EXPTHOUSE(lotchoice{ilot,2},t);
                ...
                if taxflag == 0
                    tax=sum(((1/(1+discount)).^(1:30)).*paskh,2).*basetax';
                    vactax=sum(((1/(1+discount)).^(1:30)).*vacpaskh,2).*basetax';
                elseif taxflag == 1
                    tax=sum(((1/(1+discount)).^(1:30)).*paskh,2).*...
                ...
                if taxflag == 0
                    newtax=sum(((1/(1+discount)).^(1:30)).*paskh,2).*...
                        propertytax(erun,ZONEMAP(cat(1,lotchoice{ilot,2})))';
                    c_dmg0=c_dmg/(1+deltadmg(erun)*t);
                    
                    vote = go2vote(eumdl,lccf,c_income,c_tc,c_dmg,c_good,...
                        c_hsize,c_lsize,c_amen,hsize,lsize,amen,c_pstrm,...
                        c_npstrm,paskh,newtax,tax,c_dmg0);
                    
                    votecount(t)=mat2cell([cat(1,votecount{t}); vote],...
                        length(cat(1,votecount{t}))+1,1);
                end
...
%                         mvcost(iocc(c))=mean(mvcost_util);  
            ...
            votedecision(t)=sum(cat(1,votecount{:,t}))/length(cat(1,votecount{:,t})) > 0.5;
            if taxflag == 0 && votedecision(t) == 1
                taxflag=1;
            end
...
%             lotchoice(inewlots,7)=num2cell(Paskhouse(inewlots));
            ...
            if isempty(find(inewlots,1))==0
                ...
                if taxflag == 1
                    Cdam(inewlots,t)=num2cell(Paskhouse(inewlots).*...
                        (0.01*housedam(cat(1,lotchoice{inewlots,2}))));
                elseif taxflag == 0 && stormoccur(t) == 1
                    Cdam(inewlots,t)=num2cell((1+deltadmg(erun)*t)*Paskhouse(inewlots).*...
                        (0.01*housedam(cat(1,lotchoice{inewlots,2}))));
                elseif taxflag == 0 && stormoccur(t) == 0
                    ilaststorm=find(stormoccur(t-1:-1:1) == 1,1,'first');
                    if isempty(find(ilaststorm,1)) == 1
                        Cdam(inewlots,t)=num2cell((1+deltadmg(erun)*t)*...
                            Paskhouse(inewlots).*(0.01*housedam(cat(1,lotchoice{inewlots,2}))));
                    else
                        Cdam(inewlots,t)=num2cell((1+deltadmg(erun)*(t-ilaststorm))*...
                            Paskhouse(inewlots).*(0.01*housedam(cat(1,lotchoice{inewlots,2}))));
                    end
                end
                % spatially-explicit flood insurance rates, based on Gallagher (2014), p. 210
...
%                     lotchoice{lotid,9}=(lotchoice{lotid,9} == 1 | euchoice(conid,lotid) == 2);  %mitigatoin choice indicator
            ...
            if taxflag == 0
                TAXREV(t)=sum(sum(((1/(1+discount)).^(1:30)).*...
                    cat(1,lotchoice{ifilled,7})).*basetax);
            elseif taxflag == 1
                TAXREV(t)=sum(sum(cat(1,lotchoice{ifilled,7})*((1/(1+discount)).^(1:30)),2).*...
