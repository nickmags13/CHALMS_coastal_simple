...
poolobj=parpool(10);
addAttachedFiles(poolobj,{'load_expmntlparms_event_fullinfo.m','loadempdata.m',...
    'parsave_event_fullinfo.m','distmat.m','load_farmmap.m','load_DIST2CBD_east.m',...
    'load_distmat.m','calc_prisk.m','calc_eu_fullinfo.m','calc_react_fullinfo.m'});
...
%         rndstr.Substream=mrun;
        ...
        [am0,am_slope,ampref_max,ampref_min,maxPflood,highrisk,stormfreq,maxdam,...
            ...
            eumodel,lclcoeff,altamen]=load_expmntlparms_event_fullinfo(EXPTRUNS);
        %<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
...
%         landscapefname='C:\Users\nmagliocca\Documents\Matlab_code\CHALMS_coast\simple-chalms\ilandscape.mat';
...
%         REGWGHT=1-LOCWGHT;
        ...
        lowminwage=minwage;
        WAGECLASS=3;
        HIBETA=[0.3 0.34];     %approximate max and mins for housepref come from calvert county census data
        MIDBETA=[0.35 0.39];
        LOWBETA=[0.4 0.6];
        incomeg=0.05;
        incomesigma=1;
...
%         Allfarmdist=zeros(Nfarmers,1);
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%   Distance matrices   %%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        distfname='C:\Users\nmagliocca\Documents\Matlab_code\CHALMS_coast\simple-chalms\DIST2CBD_east.mat';
        Sdist2cbd=load_DIST2CBD_east(distfname);
        farmmapfname='C:\Users\nmagliocca\Documents\Matlab_code\CHALMS_coast\simple-chalms\FARMMAP_grid.mat';
...
%         stormdraw(1:TSTART+10)=0;
        stormoccur=zeros(1,TMAX);
%         stormoccur(TSTART+10:TMAX)=(sum(Psevere) > stormdraw(TSTART+10:TMAX));
...
%                     end
        ...
        distfname='C:\Users\nmagliocca\Documents\Matlab_code\CHALMS_coast\simple-chalms\master_dist.mat';
...
%         CONINFO(:,12)=num2cell(rand(length(CONINFO(:,1)),1));
        ...
        rentmodelfname='C:\Users\nmagliocca\Documents\Matlab_code\CHALMS_coast\simple-chalms\rentmodel.mat';
...
%                 cat(1,Cdam{1:Nlots(TSTART),TSTART}));       %annualized
            ...
            for c=1:length(CONINFO(:,1))
                ...
                c_tax=coasttax(cat(1,lotchoice{:,2}));
                    ...
                    for ius=1:length(ilotmatch)
                        if euchoice(conset(cs),ilotmatch(ius)) == 1
                            ...
                            uset(ius)=CONINFO{conset(cs),13}.*...
                                ...
                                coasttax(lotchoice{ilotmatch(ius),2})-subPhousebid(conset(cs),ilotmatch(ius))'-...
                                ...
                                ((max(CONINFO{conset(cs),1}-travelcost{lotchoice{ilotmatch(ius),2}}-...
                                coasttax(lotchoice{ilotmatch(ius),2})-subPhousebid(conset(cs),ilotmatch(ius))',0).^CONINFO{conset(cs),3}).*...
                                (Lottype{ilotmatch(ius),4}.^CONINFO{conset(cs),4}).*...
                                (Lottype{ilotmatch(ius),3}.^CONINFO{conset(cs),5}).*...
                                (Lottype{ilotmatch(ius),7}.^CONINFO{conset(cs),6}));
                        elseif euchoice(conset(cs),ilotmatch(ius)) == 2
                            ...
                            uset(ius)=CONINFO{conset(cs),13}.*...
                                ((max(CONINFO{conset(cs),1}-travelcost{lotchoice{ilotmatch(ius),2}}-...
                                coasttax(lotchoice{ilotmatch(ius),2})-subPhousebid(conset(cs),ilotmatch(ius))'-Cdam{ilotmatch(ius),TSTART}-...
                                IPrate{ilotmatch(ius),TSTART}+insurecov(erun)*Cdam{ilotmatch(ius),TSTART},0).^CONINFO{conset(cs),3}).*...
                                (Lottype{ilotmatch(ius),4}.^CONINFO{conset(cs),4}).*...
                                (Lottype{ilotmatch(ius),3}.^CONINFO{conset(cs),5}).*...
                                (Lottype{ilotmatch(ius),7}.^CONINFO{conset(cs),6}))+...
                                (1-CONINFO{conset(cs),13}).*...
                                ((max(CONINFO{conset(cs),1}-travelcost{lotchoice{ilotmatch(ius),2}}-...
                                coasttax(lotchoice{ilotmatch(ius),2})-subPhousebid(conset(cs),ilotmatch(ius))'-IPrate{ilotmatch(ius),TSTART},0).^CONINFO{conset(cs),3}).*...
                                (Lottype{ilotmatch(ius),4}.^CONINFO{conset(cs),4}).*...
                                (Lottype{ilotmatch(ius),3}.^CONINFO{conset(cs),5}).*...
                                (Lottype{ilotmatch(ius),7}.^CONINFO{conset(cs),6}));
                        end
...
%             cat(1,lotchoice{ifilled,7}));
        % single house type
        estrentmdl=fitlm([cat(1,travelcost{cat(1,lotchoice{ifilled,2})}) ...
            coasttax(cat(1,lotchoice{ifilled,2})) cat(1,coastprox{cat(1,lotchoice{ifilled,2})})],...
            cat(1,lotchoice{ifilled,7}));
        
        avg_estrent(:)=predict(estrentmdl,[cat(1,travelcost{:}) coasttax...
            cat(1,coastprox{:})]);
...
%                 wtpland(iNfarmers(rf),tlearn)=FARMPROJ(iNfarmers(rf));
        ...
        for ires=1:length(ifilled)
            ...
            hopt=((CONINFO{c,1}-cat(1,travelcost{cat(1,lotchoice{ifilled(ires),2})})-...
                coasttax(cat(1,lotchoice{ifilled(ires),2}))-avgrent(:,TSTART)).^CONINFO{c,3}).*(cat(1,Lottype{ifilled(ires),4}).^...
...
%             end
            ...
            rentmdl=fitlm([cat(1,Lottype{ifilled,8}) coasttax(cat(1,lotchoice{ifilled,2})) ...
            ...
            for tl=1:length(potentialbuy)
                ...
                for lt=1:HT
                    ...
                    if z(lt,1) >= zonecheck(1) && z(lt,1) <= zonecheck(2)
                        ...
                        warning('on','all');
                        if likelotcount == 0
                            subRENTPROJ(lt)=predict(rentmdl,[travelcost{potentialbuy(tl)} ...
                                coasttax(potentialbuy(tl)) coastprox{potentialbuy(tl)} ...
                                simlots_income(lt)]);
                        else
                            ...
                            if isempty(icloselot)==1
                                ...
                                % area ...
                                locrentproj=predict(rentmdl,[travelcost{potentialbuy(tl)} ...
                                    coasttax(potentialbuy(tl)) coastprox{potentialbuy(tl)} ...
                                    simlots_income(lt)]);
                                ...
                                regrentproj=predict(rentmdl,[travelcost{potentialbuy(tl)} ...
                                    coasttax(potentialbuy(tl)) coastprox{potentialbuy(tl)} ...
                                    simlots_income(lt)]);
                                subRENTPROJ(lt)=LOCWGHT*locrentproj+...
                                    REGWGHT*regrentproj;
...
%             CONINFO(inewpop,12)=num2cell(rand(newpop,1));
            ...
            for c=1:length(iocc)
                ...
                c_tax=coasttax(cat(1,lotchoice{ilot,2}));
...
%                 paskh=EXPTHOUSE(lotchoice{ilot,2},t);
                ...
                vactax=coasttax(cat(1,lotchoice{inewlots,2}));
                vacdmg=cat(1,Cdam{inewlots,t});
                altpaskh=max(predict(rentmdl,[c_tc c_tax 0.1*amen c_income]),ccost);
                stormflag=(stormoccur(t)==1);
                eumdl=eumodel(erun);
                lccf=lclcoeff(erun);
                
                [Upick,eucheck,wtp_pick,eu_base,eu_ins,wtp_base,wtp_ins,mvcheck]=...
                    calc_react_fullinfo(eumdl,lccf,c_income,c_tc,c_dmg,c_iprate,c_good,c_hsize,...
                    ...
                    alt_coastprox,stormflag,c_tax,vactax);
...
%                 % expected utility from no action
%                 EU_base(iocc(c),ilot)=CONINFO{iocc(c),13}*...
%                     ((max(CONINFO{iocc(c),1}-cat(1,travelcost{cat(1,lotchoice{ilot,2})})-...
%                     lotchoice{ilot,7}-cat(1,Cdam{ilot,t}),0).^CONINFO{iocc(c),3}).*(cat(1,Lottype{ilot,4}).^...
...
%                     (cat(1,Lottype{ilot,7}).^CONINFO{iocc(c),6}));
%                 % expected utility from insurance
%                 EU_ins(iocc(c),ilot)=CONINFO{iocc(c),13}*...
%                     ((max(CONINFO{iocc(c),1}-cat(1,travelcost{cat(1,lotchoice{ilot,2})})-...
%                     lotchoice{ilot,7}-cat(1,Cdam{ilot,t})-cat(1,IPrate{ilot,t})+...
%                     insurecov(erun)*cat(1,Cdam{ilot,t}),0).^CONINFO{iocc(c),3}).*(cat(1,Lottype{ilot,4}).^...
%                     CONINFO{iocc(c),4}).*(cat(1,Lottype{ilot,3}).^CONINFO{iocc(c),5}).*...
%                     (cat(1,Lottype{ilot,7}).^CONINFO{iocc(c),6}))+...
%                     (1-CONINFO{iocc(c),13})*...
%                     ((max(CONINFO{iocc(c),1}-cat(1,travelcost{cat(1,lotchoice{ilot,2})})-...
%                     lotchoice{ilot,7}-cat(1,IPrate{ilot,t}),0).^CONINFO{iocc(c),3}).*(cat(1,Lottype{ilot,4}).^...
%                     CONINFO{iocc(c),4}).*(cat(1,Lottype{ilot,3}).^CONINFO{iocc(c),5}).*...
%                     (cat(1,Lottype{ilot,7}).^CONINFO{iocc(c),6}));
                
                %                     [Umax,eucheck]=max([EU_base(iocc(c),ilot); EU_ins(iocc(c),ilot)]);
                %                     euchoice(iocc(c),ilot)=eucheck;
                %                     CONINFO{iocc(c),10}=Umax;   %update utility of current residents
%                 euchoice(iocc(c),ilot)=1;
...
% %                         (cat(1,Lottype{ilot,7}).^CONINFO{iocc(c),6}));
% %                     
                        iresist_chk=(eu_ins-CONINFO{iocc(c),9} > discount*movethresh(erun)*CONINFO{iocc(c),9});
                        if iresist_chk == 0
                            dmgstress_chk(iocc(c))=0;
...
%             lotchoice(inewlots,7)=num2cell(Paskhouse(inewlots));
            ...
            for c=1:length(inewcon)
                ...
                c_tax=coasttax(cat(1,lotchoice{inewlots,2}));
...
%                 % expected utility from no action
%                 wtpcon_base(inewcon(c),inewlots)=CONINFO{inewcon(c),13}*((CONINFO{inewcon(c),1}-...
...
%                     (1-CONINFO{inewcon(c),13})*((CONINFO{inewcon(c),1}-...
%                     cat(1,travelcost{cat(1,lotchoice{inewlots,2})}))*...
%                     (CONINFO{inewcon(c),4}+CONINFO{inewcon(c),5}+CONINFO{inewcon(c),6}));
%                 EU_base(inewcon(c),inewlots)=CONINFO{inewcon(c),13}*...
%                     ((max(CONINFO{inewcon(c),1}-cat(1,travelcost{cat(1,lotchoice{inewlots,2})})-...
%                     Paskhouse(inewlots)-cat(1,Cdam{inewlots,t}),0).^CONINFO{inewcon(c),3}).*(cat(1,Lottype{inewlots,4}).^...
...
%                     ((max(CONINFO{inewcon(c),1}-cat(1,travelcost{cat(1,lotchoice{inewlots,2})})-...
%                     Paskhouse(inewlots),0).^CONINFO{inewcon(c),3}).*(cat(1,Lottype{inewlots,4}).^...
%                     CONINFO{inewcon(c),4}).*(cat(1,Lottype{inewlots,3}).^CONINFO{inewcon(c),5}).*...
%                     (cat(1,Lottype{inewlots,7}).^CONINFO{inewcon(c),6}));
%                 % expected utility from insurance
%                 wtpcon_ins(inewcon(c),inewlots)=CONINFO{inewcon(c),13}*((CONINFO{inewcon(c),1}-...
%                     cat(1,travelcost{cat(1,lotchoice{inewlots,2})})-cat(1,Cdam{inewlots,t})-...
%                     cat(1,IPrate{inewlots,t})+insurecov(erun)*cat(1,Cdam{inewlots,t}))*...
%                     (CONINFO{inewcon(c),4}+CONINFO{inewcon(c),5}+CONINFO{inewcon(c),6}))+...
%                     (1-CONINFO{inewcon(c),13})*((CONINFO{inewcon(c),1}-...
%                     cat(1,travelcost{cat(1,lotchoice{inewlots,2})})-cat(1,IPrate{inewlots,t}))*...
%                     (CONINFO{inewcon(c),4}+CONINFO{inewcon(c),5}+CONINFO{inewcon(c),6}));
%                 EU_ins(inewcon(c),inewlots)=CONINFO{inewcon(c),13}*...
%                     ((max(CONINFO{inewcon(c),1}-cat(1,travelcost{cat(1,lotchoice{inewlots,2})})-...
%                     Paskhouse(inewlots)-cat(1,Cdam{inewlots,t})-cat(1,IPrate{inewlots,t})+...
%                     insurecov(erun)*cat(1,Cdam{inewlots,t}),0).^CONINFO{inewcon(c),3}).*(cat(1,Lottype{inewlots,4}).^...
%                     CONINFO{inewcon(c),4}).*(cat(1,Lottype{inewlots,3}).^CONINFO{inewcon(c),5}).*...
%                     (cat(1,Lottype{inewlots,7}).^CONINFO{inewcon(c),6}))+...
%                     (1-CONINFO{inewcon(c),13})*...
%                     ((max(CONINFO{inewcon(c),1}-cat(1,travelcost{cat(1,lotchoice{inewlots,2})})-...
%                     Paskhouse(inewlots)-cat(1,IPrate{inewlots,t}),0).^CONINFO{inewcon(c),3}).*(cat(1,Lottype{inewlots,4}).^...
%                     CONINFO{inewcon(c),4}).*(cat(1,Lottype{inewlots,3}).^CONINFO{inewcon(c),5}).*...
%                     (cat(1,Lottype{inewlots,7}).^CONINFO{inewcon(c),6}));
% 
% %                 [Umax,eucheck]=max([EU_base(inewcon(c),inewlots); EU_ins(inewcon(c),inewlots)],[],1);
% %                 wtpstack=[wtpcon_base(inewcon(c),inewlots); wtpcon_ins(inewcon(c),inewlots)];
% %                 euchoice(inewcon(c),inewlots)=eucheck;
% %                 U(inewcon(c),inewlots)=Umax;
% %                 for iw=1:length(wtpstack(1,:))
% %                     wtpcon(inewcon(c),inewlots(iw))=wtpstack(eucheck(iw),iw);
% %                 end
%                 % turn-off insurance option
%                 U(inewcon(c),inewlots)=EU_base(inewcon(c),inewlots);
%                 euchoice(inewcon(c),inewlots)=1;
%                 wtpcon(inewcon(c),inewlots)=wtpcon_base(inewcon(c),inewlots);
            ...
            for c=1:length(inewcon)
                ...
                if find(ismember(inewcon(c),conreloc)) == 1
                    iucheck=(CONINFO{inewcon(c),10} >= CONINFO{inewcon(c),13}*...
                        ((max(CONINFO{inewcon(c),1}-cat(1,travelcost{cat(1,lotchoice{inewlots,2})})-...
                        coasttax(cat(1,lotchoice{inewlots,2}))-Phousebid(inewcon(c),inewlots)'-cat(1,Cdam{inewlots,t}),0).^...
                        CONINFO{inewcon(c),3}).*(cat(1,Lottype{inewlots,4}).^...
                        CONINFO{inewcon(c),4}).*(cat(1,Lottype{inewlots,3}).^CONINFO{inewcon(c),5}).*...
                        (cat(1,Lottype{inewlots,7}).^CONINFO{inewcon(c),6}))+...
                        (1-CONINFO{inewcon(c),13})*...
                        ((max(CONINFO{inewcon(c),1}-cat(1,travelcost{cat(1,lotchoice{inewlots,2})})-...
                        coasttax(cat(1,lotchoice{inewlots,2}))-Phousebid(inewcon(c),inewlots)',0).^CONINFO{inewcon(c),3}).*(cat(1,Lottype{inewlots,4}).^...
                        CONINFO{inewcon(c),4}).*(cat(1,Lottype{inewlots,3}).^CONINFO{inewcon(c),5}).*...
                        (cat(1,Lottype{inewlots,7}).^CONINFO{inewcon(c),6})));
            ...
            while openhouse > 0
                ...
                for cs=1:length(conset)
                    ilotmatch=find(wincon==conset(cs));
                    uset=zeros(length(ilotmatch),1);
%                     if find(ismember(conset(cs),conreloc)) == 1
%                         keyboard
%                     end
                    for ius=1:length(ilotmatch)
                        if euchoice(conset(cs),inewlots(ilotmatch(ius))) == 1
                            ...
                            uset(ius)=CONINFO{conset(cs),13}.*...
                                ...
                                coasttax(lotchoice{inewlots(ilotmatch(ius)),2})-subPhousebid(conset(cs),inewlots(ilotmatch(ius)))'-...
                                ...
                                (1-CONINFO{conset(cs),13}).*...
                                ((max(CONINFO{conset(cs),1}-travelcost{lotchoice{inewlots(ilotmatch(ius)),2}}-...
                                coasttax(lotchoice{inewlots(ilotmatch(ius)),2})-subPhousebid(conset(cs),inewlots(ilotmatch(ius)))',0).^CONINFO{conset(cs),3}).*...
                                (Lottype{inewlots(ilotmatch(ius)),4}.^CONINFO{conset(cs),4}).*...
                                (Lottype{inewlots(ilotmatch(ius)),3}.^CONINFO{conset(cs),5}).*...
                                (Lottype{inewlots(ilotmatch(ius)),7}.^CONINFO{conset(cs),6}));
                        elseif euchoice(conset(cs),inewlots(ilotmatch(ius))) == 2
                            ...
                            uset(ius)=CONINFO{conset(cs),13}.*...
                                ((max(CONINFO{conset(cs),1}-travelcost{lotchoice{inewlots(ilotmatch(ius)),2}}-...
                                coasttax(lotchoice{inewlots(ilotmatch(ius)),2})-subPhousebid(conset(cs),inewlots(ilotmatch(ius)))'-...
                                Cdam{inewlots(ilotmatch(ius)),t}-IPrate{inewlots(ilotmatch(ius)),t}+...
                                insurecov(erun)*Cdam{inewlots(ilotmatch(ius)),t},0).^CONINFO{conset(cs),3}).*...
                                (Lottype{inewlots(ilotmatch(ius)),4}.^CONINFO{conset(cs),4}).*...
                                (Lottype{inewlots(ilotmatch(ius)),3}.^CONINFO{conset(cs),5}).*...
                                (Lottype{inewlots(ilotmatch(ius)),7}.^CONINFO{conset(cs),6}))+...
                                (1-CONINFO{conset(cs),13}).*...
...
%                     lotchoice{lotid,9}=(lotchoice{lotid,9} == 1 | euchoice(conid,lotid) == 2);  %mitigatoin choice indicator
                    ...
                    lotchoice{lotid,9}=(euchoice(conid,lotid) == 2);  %mitigatoin choice indicator
...
%                         discount*subplandinfo(cat(1,lotchoice{brokerind,2})).*cat(1,Lottype{brokerind,3}));
            ...
            for ires=1:length(ifilled)
                c=lotchoice{ifilled(ires),5};
                hopt=((CONINFO{c,1}-cat(1,travelcost{cat(1,lotchoice{ifilled(ires),2})})-...
                    coasttax(cat(1,lotchoice{ifilled(ires),2}))-avgrent(:,t)).^CONINFO{c,3}).*(cat(1,Lottype{ifilled(ires),4}).^...
                    CONINFO{c,4}).*(cat(1,Lottype{ifilled(ires),3}).^CONINFO{c,5}).*...
                    (cat(1,Lottype{ifilled(ires),7}).^CONINFO{c,6});
        ...
        parsave_event_fullinfo(savefname,...
