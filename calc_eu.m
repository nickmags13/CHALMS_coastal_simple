function [Upick,eucheck,wtp_pick,eu_base,eu_ins,wtp_base,wtp_ins]=...
    ...
    c_lsize,c_amen,hsize,lsize,amen,c_pstrm,c_npstrm,paskh,icov,ddct,tax)
...
eu_base=c_pstrm.*((max(c_income-c_tc-paskh-paskh.*tax-c_dmg,0).^c_good).*...
    (hsize.^c_hsize).*(lsize.^c_lsize).*(amen.^c_amen))+...
    c_npstrm.*((max(c_income-c_tc-paskh-paskh.*tax,0).^c_good).*...
    (hsize.^c_hsize).*(lsize.^c_lsize).*(amen.^c_amen));

% expected utility from insurance
wtp_ins=c_pstrm.*(c_income-c_tc-c_dmg-c_iprate-ddct+min(icov,min(c_dmg,paskh))).*...
    (c_hsize+c_lsize+c_amen)+c_npstrm.*(c_income-c_tc-c_iprate).*...
    (c_hsize+c_lsize+c_amen);
eu_ins=c_pstrm.*((max(c_income-c_tc-paskh-paskh.*tax-c_dmg-c_iprate-ddct+...
    min(icov,min(c_dmg,paskh)),0).^c_good).*...
    (hsize.^c_hsize).*(lsize.^c_lsize).*(amen.^c_amen))+...
    c_npstrm.*((max(c_income-c_tc-paskh-paskh.*tax-c_iprate,0).^c_good).*...
    (hsize.^c_hsize).*(lsize.^c_lsize).*(amen.^c_amen));

wtp_pick=zeros(1,length(wtp_base));
eu_pick=zeros(1,length(wtp_base));
if eumdl == 1
...
elseif eumdl == 3
    ...
    refpt=(max(c_income-c_tc-paskh-paskh.*tax,0).^c_good).*...
        (hsize.^c_hsize).*(lsize.^c_lsize).*(amen.^c_amen); %no storm, no mitigation
    nstrm_mit=(max(c_income-c_tc-paskh-paskh.*tax-c_iprate,0).^c_good).*...
        (hsize.^c_hsize).*(lsize.^c_lsize).*(amen.^c_amen); %no storm, with mitigation
    strm_mit=(max(c_income-c_tc-paskh-paskh.*tax-c_dmg-c_iprate-ddct+min(icov,min(c_dmg,paskh)),0).^c_good).*...
        (hsize.^c_hsize).*(lsize.^c_lsize).*(amen.^c_amen); %storm, with mitigation
    strm_nomit=(max(c_income-c_tc-paskh-paskh.*tax-c_dmg,0).^c_good).*...
        (hsize.^c_hsize).*(lsize.^c_lsize).*(amen.^c_amen); %storm, no mitigation
    value_nstrm=abs(nstrm_mit-refpt)./(abs(nstrm_mit)+abs(refpt)+1);
    value_strm=abs(strm_mit-strm_nomit)./(abs(strm_nomit)+abs(strm_mit)+1);
