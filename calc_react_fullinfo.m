...
eu_ins=c_pstrm.*((max(c_income-c_tc-c_tax-paskh-c_dmg-c_iprate+icov*c_dmg,0).^c_good).*...
    (hsize.^c_hsize).*(lsize.^c_lsize).*(amen.^c_amen))+...
...
if stormflag == 1
    if isempty(find(vacpaskh,1)) == 1
        ...
        wtp_mov=c_pstrm.*(c_income-vactc-vactax-discount*c_mov-vacdmg).*(c_hsize+c_lsize+c_amen)+...
            c_npstrm.*(c_income-vactc-vactax-discount*c_mov-vacdmg).*(c_hsize+c_lsize+c_amen);
        eu_mov=c_pstrm.*(max(c_income-vactc-vactax-vacpaskh-discount*c_mov-vacdmg,0).^c_good).*...
            (vachsize.^c_hsize).*(vaclsize.^c_lsize).*(vacamen.^c_amen)+...
            c_npstrm.*(max(c_income-vactc-vactax-vacpaskh-discount*c_mov,0).^c_good).*...
            (vachsize.^c_hsize).*(vaclsize.^c_lsize).*(vacamen.^c_amen);
    end
else
    wtp_mov=0;
...
%     value_base=salwght_nstrm*refpt+salwght_strm*(refpt-c_dmg);
    ...
    refpt=(max(c_income-c_tc_c_tax-paskh,0).^c_good).*...
        (hsize.^c_hsize).*(lsize.^c_lsize).*(amen.^c_amen); %no storm, no mitigation
    ...
    if isempty(find(vacpaskh,1)) == 1
        ...
        nstrm_mov=mean((max(c_income-vactc-vactax-vacpaskh-discount*c_mov,0).^c_good).*...
            (vachsize.^c_hsize).*(vaclsize.^c_lsize).*(vacamen.^c_amen)); %no storm, with move cost
        strm_mov=mean((max(c_income-vactc-vactax-vacpaskh-discount*c_mov,0).^c_good).*...
            (vachsize.^c_hsize).*(vaclsize.^c_lsize).*(vacamen.^c_amen)); %storm, same as no storm
    end
    value_strm=abs((strm_mit+strm_mov+alt_mov)/3-strm_nomit)./...
        (abs(strm_nomit)+abs((strm_mit+strm_mov+alt_mov)/3)+1);
    value_nstrm=abs((nstrm_mit+nstrm_mov+alt_mov)/3-refpt)./...
        (abs((nstrm_mit+nstrm_mov+alt_mov)/3)+abs(refpt)+1);
